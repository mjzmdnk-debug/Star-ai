...
(() => {
  ...
  function applyLanguage(lang) {
    ...
    setText('newChat', t.newChat);
    setText('logout', t.logout);
    setText('creditsSide', get('creditsSide')?.textContent || '—');
    ...
    const quick = document.querySelectorAll('.star-quick-tool'); if (quick.length >= 4) { quick[0].textContent = '✨ ' + t.improve; quick[1].textContent = '💡 ' + t.ideas; quick[2].textContent = '📝 ' + t.summary; quick[3].textContent = '🌐 ' + t.translate; }
    const select = get('starLanguageSelect'); if (select) select.value = lang;
    localStorage.setItem('star-ai-language', lang);
  }
  function init() {
    if (get('starLanguageSelect')) { applyLanguage(localStorage.getItem('star-ai-language') || 'tr'); return; }
    ...
    const select = get('starLanguageSelect'); select.addEventListener('change', () => applyLanguage(select.value));
