...
if (!server.includes('STAR_AI_WEB_RESEARCH_V1')) {
  const start = server.indexOf("app.post('/api/chat'");
  const end = server.indexOf("\napp.get('/api/admin/overview'", start);
  if (start < 0 || end < 0) throw new Error('STAR_AI_WEB_RESEARCH_V1: chat endpoint boundaries not found');
  ...
  const replacement = `\n  const preferredModel = model || AI_MODEL;\n  const researchText = String(message || '').toLowerCase();\n  const needsWebResearch = /(güncel|bugün|şimdi|son dakika|son gelişme|en son|2026|haber|fiyat|kur|borsa|piyasa|maç|skor|takvim|araştır|araştırma|bilimsel|makale|çalışma|kaynak|istatistik|veri|current|today|latest|recent|news|price|market|research|study|paper|source|statistics)/i.test(researchText);`;
  ...
  endpoint = endpoint.replace(answerMarker, answerReplacement);

  if (!endpoint.includes("type: 'web_search'")) throw new Error('STAR_AI_WEB_RESEARCH_V1: web search injection failed');
  server = server.slice(0, start) + endpoint + server.slice(end);
  server = server.replace('// STAR_AI_PRO_ENGINE_V2', '// STAR_AI_PRO_ENGINE_V2\n// STAR_AI_WEB_RESEARCH_V1');
  fs.writeFileSync(serverFile, server);
  console.log('STAR AI live web research engine enabled.');
}
