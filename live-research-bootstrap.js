...
if (!hasResponsesResearch) {
  ...
  const oldCreatePattern = /    const completion = await openai\.chat\.completions\.create\(\{[\s\S]*?    if \(!answer\) throw new Error\('Empty AI response'\);/;
  ...
  if (oldCreatePattern.test(endpoint)) {
    endpoint = endpoint.replace(oldCreatePattern, newCreate);
